...
export default function CinematicCanvas() {
    ...
    useEffect(() => {
        if (!containerRef.current || isReducedMotionRef.current || !isMobile) return;

        const container = containerRef.current;
        const { PHASE2_END, PHASE3_START, PHASE3_END } = MOBILE_SCROLL;
        const scaleAtPhase2End = 1.08;
    ...
    useEffect(() => {
        if (!canvasWrapperRef.current || isReducedMotionRef.current || !isMobile) return;

        const wrapper = canvasWrapperRef.current;
        const { PHASE3_START, PHASE3_END } = MOBILE_SCROLL;
